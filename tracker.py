...
class _LinearTrack:
    ...
    search_radius 根据最近一次检测的 bbox 尺寸动态计算（search_diameters × ball_d_px），
    以自动适应近大远小的透视变化。
    """
    _next_id = 0
    ...
    def __init__(self, det, frame_idx, min_hits, search_diameters, max_dist):
        ...
        self._ball_d_px         = (x2 - x1 + y2 - y1) / 2.0
    ...
    def search_radius(self) -> float:
        """当前搜索半径：search_diameters × 最近检测到的球径（px）。"""
        return self._search_diameters * self._ball_d_px

    @property
    def effective_gate(self) -> float:
        """搜索门限：仅 1 个历史点时无方向信息，用最大球速作为门限；否则用拟合预测半径。"""
        if len(self.history) == 1 and self._max_dist is not None:
...
def _match(tracks, dets, max_dist, min_iou=None):
    """
    匈牙利算法匹配轨迹与检测。

    max_dist != None → 欧氏中心距离模式（网球）
                       可为标量（所有轨迹统一门限）或长度等于 tracks 的列表（逐轨迹门限）
    max_dist == None → IoU 模式（球员 / 球拍）
    ...
    if max_dist is not None:
        ...
        for i, t in enumerate(tracks):
            ...
            for j, d in enumerate(dets):
                dcx, dcy = _center(d['bbox'])
                cost[i, j] = ((tcx - dcx)**2 + (tcy - dcy)**2) ** 0.5
...
class Tracker:
    ...
    def step(self, detections, frame_idx):
        ...
        matched1, unmatched_t1, unmatched_d_high = _match(self._tracks, dets_high, gates1)
        ...
        if unmatched_t1 and dets_low:
            ...
            matched2_local, _, _ = _match(tracks2, dets_low, gates2)
