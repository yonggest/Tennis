...
输出：`<video>_tracked.json`（同 COCO 格式，每个球标注加 `track_id` 字段，gap 帧线性插值补全）
...
annotations  : [{id, image_id, category_id, bbox [x,y,w,h], area, iscrowd, score,
                 ...
                 interpolated?,  ← track.py gap 插值帧标记（bool）
...
## 球追踪算法（track.py / tracker.py）

### 预过滤

每帧球检测先按 bbox 面积和宽高比过滤，排除过大/过小/过扁的误检。面积范围和宽高比阈值由帧率和像素/米比例从物理常量推算（`BallTracker.from_video`）。

### SORT 风格追踪

两阶段 ByteTrack 风格匹配（高置信度先匹配，低置信度只续接已有轨迹）+ 匈牙利算法。

**预测**：每条轨迹用最近 `_LINEAR_WINDOW=4` 个历史点线性外推下一帧位置（不足时取全部）。
